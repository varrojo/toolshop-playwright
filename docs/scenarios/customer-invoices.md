...
Status: 🟡 In progress — 9 of 10 scenarios done.
...
- Logged-out access to `/account/invoices` does redirect to `/auth/login`, but with a noticeable delay (~2-4s, not immediate) — needs a generous timeout on the URL assertion rather than the default, or it'll flake.
...
- Scenario #7 uses a new `'User from Seeded Accounts'` describe block (`customer-invoices.spec.ts`) logging in with a site-provided seeded account (`SEEDED_USER`) instead of a fresh `registeredUser` + checkout — avoids the checkout flow's `#order-confirmation` flake entirely (see below) and needs whatever invoice is already on that account.
- Since a seeded/shared account's invoice list isn't test-controlled, the test reads the first invoice dynamically (`invoicesPage.allInvoiceNumbers()[0]`) rather than hardcoding a specific invoice number — same reasoning as scenario #10's pagination approach above.
- Login credentials (`ADMIN_EMAIL`/`ADMIN_PASSWORD` for stock management, `SEEDED_USER_EMAIL`/`SEEDED_USER_PASSWORD` for scenario #7) live in a gitignored `.env`, loaded via `dotenv` in `playwright.config.ts`; `.env.example` documents the required keys. `InvoicesPage.ts` throws immediately at load time (`requireEnv` helper) if a required variable is missing, rather than silently passing `undefined` into `login()`.
